        ...
        <div class="w-[calc(100%-30px)] flex">
          ...
          <div class="w-[200px] ">
            <div class="flex items-center justify-start space-x-4" @click="toggleDrop">
              ...
              <div class="font-semibold text-left dark:text-gray-400">
                <div>{{ userData ? userData.full_name : 'User' }}</div>
                <div v-if="userData" >
                  <div v-if="userData.is_student" class="text-xs text-gray-500 dark:text-gray-400">Học sinh</div>
                  <div v-if="userData.is_parent" class="text-xs text-gray-500 dark:text-gray-400">Phụ huynh</div>
                  <div v-if="userData.is_teacher" class="text-xs text-gray-500 dark:text-gray-400">Giáo viên</div>
                  <div v-if="userData.is_admin" class="text-xs text-gray-500 dark:text-gray-400">ADMIN</div>
                </div>
            ...
            <div v-show="showDropDown" class="absolute right-[10px] z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
              <div class="py-1 text-left" role="none">
                ...
                <router-link :to="{ name: 'detail', params: {userData: userData} }">
                  Thông tin tài khoản
                </router-link>
...
export default {
  ...
  methods: {
    getUserData() {
      ...
      .then(response => {
        this.userData = response.data;  // Lưu dữ liệu người dùng vào userData
    ...
    logout() {
      ...
      this.$notify({
          ...
          type: 'warn',  // Kiểu thông báo: 'success', 'error', 'warn'
          duration: 3000,
        });
